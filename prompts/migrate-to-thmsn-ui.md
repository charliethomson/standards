...
Context: `@thmsn/ui` is the fleet's shared shadcn/Radix library (new-york style, HSL
CSS-variable tokens; Tailwind v4 AND v3 consumers supported). It ships primitives (Button,
...
IconButton, EmptyState, Page, Avatar, Monogram), a v4 `theme.css`, a v3 Tailwind preset, and
a default `tokens.css`. Components are function/`forwardRef` with `data-slot` attributes; controlled
...
2. Tailwind + tokens — pick the path for the app's Tailwind version (prefer v4; migrating
   the app to Tailwind v4 as part of this work is encouraged):
   - Tailwind v4 (CSS-first, PREFERRED): one import in the CSS entry, after tailwindcss —
     it provides the theme mapping, dark variant, @source registration of the library's
     dist, tw-animate-css, and base styles. No tailwind.config needed; delete one that only
     restates the shared theme:
         @import "tailwindcss";
         @import "@thmsn/ui/theme.css";
   - Tailwind v3 (config-file, legacy): in tailwind.config.*, add the preset and scan the
     package's classes:
         presets: [require('@thmsn/ui/tailwind-preset')]   // or the ESM import
         content: [..., './node_modules/@thmsn/ui/dist/**/*.js']
     and import the tokens once at the app root, BEFORE your @tailwind directives:
         @import '@thmsn/ui/tokens.css';
   - Both paths: if the app has a generated branding tokens.css, import it AFTER the
     library's theme.css / tokens.css (same variable names → it overrides the defaults).
     Reconcile any gaps (surface / success / warning / …).
   - Remove the app's now-duplicated shadcn colour/token config where it fully overlaps the
     shared theme/preset. Keep only genuinely app-specific extensions.
